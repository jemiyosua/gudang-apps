...
func Menu(c *gin.Context) {
	...
	if string(bodyString) == "" {
		...
		dataLogMenu(jMenuResponses, reqBody.Username, errorCode, errorMessage, totalRecords, totalPage, method, path, ip, logData, allHeader, bodyJson, c)
		return
	}
		...
		if err := c.ShouldBindJSON(&reqBody); err != nil {
			...
			if method == "INSERT" {
				...
				if menuStatus != "" {
					...
					queryWhere += fmt.Sprintf(" status = %s ", menuStatus)
				}
				...
				query1 := fmt.Sprintf(`SELECT id, menu, href, status, tgl_input FROM db_menu %s %s`, queryWhere, queryLimit)
